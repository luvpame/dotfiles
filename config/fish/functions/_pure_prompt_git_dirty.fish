function _pure_prompt_git_dirty
    ...
    set --local is_git_dirty (
        ...
        if command env GIT_OPTIONAL_LOCKS=0 git rev-list --max-count=1 HEAD -- >/dev/null 2>&1;
            not command env GIT_OPTIONAL_LOCKS=0 git diff-index --ignore-submodules --cached --quiet HEAD -- >/dev/null 2>&1;
        else;
            not command env GIT_OPTIONAL_LOCKS=0 git diff --staged --ignore-submodules --no-ext-diff --quiet --exit-code >/dev/null 2>&1;
        end
        or not command env GIT_OPTIONAL_LOCKS=0 git diff --ignore-submodules --no-ext-diff --quiet --exit-code >/dev/null 2>&1
        or command env GIT_OPTIONAL_LOCKS=0 git ls-files --others --exclude-standard --directory --no-empty-directory --error-unmatch -- ':/*' >/dev/null 2>&1
        and echo "true"
    )
